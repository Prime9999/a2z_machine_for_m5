...
extern int zd_optind;
extern const char *zd_optarg;
...
#define GETOPT_SET    "mvzhy?l:c:k:r:t:" 
   ...
   {                            
      ...
      {
         case 'l':             /* lines */
            screen_rows = atoi( zd_optarg );
            break;
         case 'c':             /* columns */
            screen_cols = atoi( zd_optarg );
            break;
         case 'r':             /* right margin */
            right_margin = atoi( zd_optarg );
            break;
         case 't':             /* top margin */
            top_margin = atoi( zd_optarg );
            break;
         ...
         case 'f':
            default_fg = atoi( zd_optarg );
            break;
         case 'b':
            default_bg = atoi( zd_optarg );
            break;
#endif
#if defined OS2 || defined __MSDOS__ 
         case 'm':             /* monochrome */
         ...
         case 's':             /* strictz reporting mode */
            strictz_report_mode = atoi( zd_optarg );
   ...
   if ( errflg || zd_optind + expected_args != argc )
   {
