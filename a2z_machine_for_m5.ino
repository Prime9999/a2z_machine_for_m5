...
#else
#if (!defined(QSPI_FLASH_SECTOR_SIZE))
#define QSPI_FLASH_SECTOR_SIZE (4 * 1024)
#endif
static uint8_t cache[1024 * 1024];
#endif
...
#else
  memset(cache + cache_addr, 0, QSPI_FLASH_SECTOR_SIZE);
  memcpy(cache + cache_addr, cache_buf, QSPI_FLASH_SECTOR_SIZE);
#endif
...
#else
      memcpy(cache_buf, cache + page_addr, QSPI_FLASH_SECTOR_SIZE);
#endif
    }
...
#else
    if ( dst_off ) memcpy(dst, cache + addr, dst_off);
#endif
...
#else
    if ( copied < count ) memcpy(dst + copied, cache + (addr + copied), count - copied);
#endif
  }
  else
  {
...
//  sprintf(storyfile,"%s/%s",GAMEPATH, storyfilelist[storynum]);
  sprintf(storyfile,"%s",storyfilelist[storynum]);
