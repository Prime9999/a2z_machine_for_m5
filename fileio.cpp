...
char *zd_magic = ( char * ) MAGIC_STRING; /* mol */
...
int analyze_exefile( void )
{
   ...
   while ( ( c = gfp.read() ) > -1 )
   {
      if ( c != zd_magic[i] )
      {
         if ( c == zd_magic[0] )
         {
            i = 1;              /* Next character should match zd_magic[1] */
         }
         else
         {
            i = 0;
...
void open_story( const char *storyname )
{
   ...
   else
   {
      ...
      story_offset =
            zd_magic[MAGIC_END + 1] + zd_magic[MAGIC_END + 2] * 256L + zd_magic[MAGIC_END + 3] * 65536L;
   }
...
static int save_restore( const char *file_name, int flag )
{
   ...
   {
      //tfp = SD.open( file_name, ( flag == GAME_SAVE ) ? FILE_CREATE : FILE_READ );
